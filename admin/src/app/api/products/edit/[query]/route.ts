...
export async function POST(request: Request,{params}:{params:{query:string}}) {
    ...
    if (query.length <= 1 && !query[1].includes('name=')) {
        return new Response(JSON.stringify({error:'name parameter is missing or is not the second parameter'}),{status:400,headers:{
            'Content-Type':'application/json'
        }})
    }

    if (query.length <= 2 && !query[2].includes('description=')) {
        return new Response(JSON.stringify({error:'description parameter is missing or is not the third parameter'}),{status:400,headers:{
            'Content-Type':'application/json'
        }})
    }

    if (query.length <= 3 && !query[3].includes('price=')) {
        return new Response(JSON.stringify({error:'price parameter is missing or is not the fourth parameter'}),{status:400,headers:{
            'Content-Type':'application/json'
        }})
    }

    if (query.length <= 4 && !query[4].includes('category=')) {
        return new Response(JSON.stringify({error:'category parameter is missing or is not the fourth parameter'}),{status:400,headers:{
            'Content-Type':'application/json'
        }})
    }

    if (query.length <= 5 && !query[5].includes('properties=')) {
    ...
    const name = query[1].replace('name=','');
    const description = query[2].replace('description=','');
    const price = query[3].replace('price=','');
    const category = query[4].replace('category=','');
    const properties = query[5].replace('properties=','');
    ...
    try {
        ...
        await Product.updateOne({_id:id},{ name, description, price, images:[...images,...imageLinks], category, properties:props });
