...
/** Card Grid component */
export const CardGrid: React.FC<CardListProps> = ({items,section,paginate=false,pageLength=10}) => {
    ...
    if (paginate) {
        ...
        for (let i = minPage; i <= maxPage; i++) {
            pageElements.push(<button onClick={(e)=>setCurrentPage(i)} style={{borderRadius: '50%'}} key={i} className={`flex-shrink-0 text-lg ${currentPage === i ? 'bg-orange-500 text-white' : 'text-black dark:text-white'} flex flex-row items-center justify-center h-8 w-8`}>{i}</button>) 
        }
    ...
    return <div> 
        <div className="grid mb-8 grid-flow-row gap-x-4 card-grid max-sm:hidden text-neutral-600 sm:grid-cols-2 md:grid-cols-3">
            {paginate ? items.filter((item,index)=>(index >= (currentPage-1) * pageLength && index < currentPage * pageLength)).map((item,index)=>{
                            ...
                            <figcaption className="p-4">
                                {section === SectionEnum.FLASH_SALES ? 
                                    <div className="flex flex-row items-start justify-between">
                                        <div>
                                            <div className="border text-sm rounded-sm text-white bg-orange-500 p-1 w-fit border-orange-500">{item.oldPrice ? Math.round(((item.oldPrice-(item.price ? item.price : 0))/item.oldPrice)*100).toString()+"% off": null}</div>
                                ...
                                <div className="flex flex-row items-center gap-x-2">
                                    ...
                                    {section === SectionEnum.FLASH_SALES ? <p className="text-sm line-through text-gray-800 dark:text-gray-300">Was: {`$`+item.oldPrice}</p> : null}
                                </div>
                ...
                >
                        <figure>
                            <a href="">
                                <img alt={item.title} src={item.image} className="rounded-t h-48 w-full object-cover dark:border-b-0 border-b border-b-gray-600" />
                            </a>
                            {section !== SectionEnum.CATEGORIES ?
                            <figcaption className="p-4">
                                {section === SectionEnum.FLASH_SALES ? 
